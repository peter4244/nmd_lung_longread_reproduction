...
```bash
for f in $(curl -s https://zenodo.org/api/records/21544336/files \
           | python3 -c 'import json,sys; [print(e["key"]) for e in json.load(sys.stdin)["entries"]]'); do
  curl -sL -o "$f" "https://zenodo.org/api/records/21544336/files/$f/content"
done
...
All 58 entries should report `OK`. Two failure modes look alike and are not:

- `58 listed files could not be read` — wrong working directory, not a bad download.
- lines reading `FAILED` — you may have downloaded **HTML**. A restricted or unauthenticated Zenodo
  fetch returns HTTP 200 with a cover page, so `curl` exits 0 and writes a plausible file. Check with
  `head -c 20 MANIFEST.sha256`: it should be hex, not `<!DOCTYPE html>`. If it is HTML you do not
  have access to the record, and retrying will not help.
...
30 s under `--mem=96G`. Every other step in this section runs on a laptop.
...
reads `h5["w500/stop_windows"]`, and a current HDF5 contains `w100`, `w1000` and `w2000` with
datasets named `atg_codes`/`stop_codes` — both the group and the dataset name belong to an earlier
schema, so it exits 1 with `KeyError: 'stop_windows' doesn't exist`. It also has nothing to do:
...
before anything is tested. The repository's own `drivers/slurm_determinism.sh` omits it too.
