...
PTC+/PTC- mechanism dichotomy. Pre-registered in ./RATIONALE.md.

Data:
  ./data/panelA_5utr_length_long.tsv (group, utr5_length_nt per isoform)
  ./data/panelA_5utr_length_descriptives.tsv (group n/median/IQR)
  ./data/panelA_5utr_length_pairwise.tsv (Wilcoxon HL shift + Cliff δ + p)
"""
...
def build_figure():
    long = pd.read_csv(HERE.parent / "data" / "panelA_5utr_length_long.tsv", sep="\t")
    stats = pd.read_csv(HERE.parent / "data" / "panelA_5utr_length_pairwise.tsv", sep="\t")
    desc = pd.read_csv(HERE.parent / "data" / "panelA_5utr_length_descriptives.tsv", sep="\t")
    ...
    # Significance brackets above
    ymax = max(d.max() for d in data)
    y_top = ymax + 0.18
    spacing = 0.50
    # Plot 3 pairwise comparisons: PTC- vs PTC+, PTC- vs Control, PTC+ vs Control
    ...
    for gx, gy, ix, iy, yy in pairs:
        ...
        ax.text((ix + iy) / 2, yy + 0.02, marker,
...
def main():
    ...
    out_dir = HERE.parent
    fig.savefig(out_dir / "figure4_panelA_5utr_length_all3enst.pdf", facecolor="white")
    fig.savefig(out_dir / "figure4_panelA_5utr_length_all3enst.png", dpi=300, facecolor="white")
    print("Saved: figure4_panelA_5utr_length_all3enst.{pdf,png}")
