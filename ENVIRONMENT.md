...
`renv.lock` and both conda environments, so it either succeeds or fails loudly. This path works
...
So run **`tools/assert_container_isolation.R` at the head of every stage**; it asserts `$HOME` is
*empty*, which is what actually proves isolation, and that no authoring tree is reachable.
...
**Verify the image itself** with `tools/verify_container.sh --runtime apptainer --image <sif>`: R at
